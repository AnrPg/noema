...
(Neo4j, Prisma, Redis cache) for the domain interfaces defined in Phase 3.
Phase 5 introduces the **service layer** — the `KnowledgeGraphService` class
that orchestrates repository calls, validates inputs, enforces edge policies,
...
`updateEdge`, `listEdges`) but the `IKnowledgeGraphService` interface from
Phase 3 only had `createEdge` and `deleteEdge`.
...
triples, and include them in both the operation log and the domain event payload.
...
with `markStale`, `isStale`, and `getStalenessRecord` methods. The service
layer calls `markStale` after every mutating operation (non-blocking — failures
...
These are declared as non-async (no `await`) to satisfy `@typescript-eslint/require-await`.

## Consequences

### Positive

- Complete PKG CRUD surface: 5 node methods + 5 edge methods + 4 traversal + 3 CKG read
