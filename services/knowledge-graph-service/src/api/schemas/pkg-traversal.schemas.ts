...
import { GraphEdgeTypeSchema, GraphNodeTypeSchema, NodeIdSchema, UserIdSchema } from '@noema/validation';
import { z } from 'zod';
 ...
 */
export function parseEdgeTypesFilter(
  edgeTypesParam: string | undefined
): string[] | undefined {
  if (!edgeTypesParam) return undefined;
  const types = edgeTypesParam.split(',').map((t) => t.trim()).filter(Boolean);
 ...
 * Returns undefined if the input is undefined or empty.
 */
export function parseNodeTypesFilter(
  nodeTypesParam: string | undefined
): string[] | undefined {
  if (!nodeTypesParam) return undefined;
  const types = nodeTypesParam.split(',').map((t) => t.trim()).filter(Boolean);
  for (const t of types) {
