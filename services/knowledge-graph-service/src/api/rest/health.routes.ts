/**
 * @noema/knowledge-graph-service - Health Routes
 *
 * Health check endpoints for Kubernetes probes.
 * Uses standardized types from @noema/contracts.
 * Checks PostgreSQL, Neo4j, and Redis connectivity.
 */
...
export function registerHealthRoutes(
  ...
  fastify.get<{ Reply: IHealthCheckResponse }>(
    ...
    async (_request, reply) => {
      ...
      const neo4jCheck = await checkDependency(async () => {
      ...
      const anyDown =
        dbCheck.status === 'down' || neo4jCheck.status === 'down' || redisCheck.status === 'down';
      ...
      const response: IHealthCheckResponse = {
        ...
        checks: {
          ...
          graphDatabase: {
            status: neo4jCheck.status,
            latencyMs: neo4jCheck.latencyMs,
            checkedAt: now,
            ...(neo4jCheck.error !== undefined ? { error: neo4jCheck.error } : {}),
          },
          redis: {
