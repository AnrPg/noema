...
export function registerToolRoutes(
  ...
  fastify.get('/v1/tools', { preHandler: authMiddleware }, async (request, reply) => {
    if (
      !hasRequiredScopes(request.user as IScopeUser | undefined, {
        requiredScopes: ['metacognition:tools:read'],
        match: 'any',
      })
    ) {
  ...
  fastify.post('/v1/tools/execute', { preHandler: authMiddleware }, async (request: FastifyRequest, reply: FastifyReply) => {
    if (
      !hasRequiredScopes(request.user as IScopeUser | undefined, {
        requiredScopes: ['metacognition:tools:execute'],
        match: 'any',
      })
    ) {
      await reply.status(403).send({
        error: { code: 'FORBIDDEN_MISSING_SCOPE', message: 'Missing required scope for tool execution' },
        metadata: buildMetadata(request),
      });
      return;
    }
    ...
    if (
      ...
      !hasRequiredScopes(request.user as IScopeUser | undefined, definition.scopeRequirement)
    ) {
      await reply.status(403).send({
        error: { code: 'FORBIDDEN_MISSING_SCOPE', message: 'Missing required scope for tool execution' },
        metadata: buildMetadata(request),
    ...
    const result = await toolRegistry.execute(body.tool, body.input ?? {}, user?.sub ?? '', request.id);
