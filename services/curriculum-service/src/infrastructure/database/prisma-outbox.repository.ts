...
import type {
  ...
  IOutboxEventRecord,
  IOutboxRepository,
} from '../../domain/curriculum-service/outbox.repository.js';
...
function toOutboxDomain(
  ...
  return {
    ...
    payload: row.payload,
    metadata: row.metadata as IOutboxEventRecord['metadata'],
...
export class PrismaOutboxRepository implements IOutboxRepository {
  ...
  async markDeadLettered(id: EventId, claimOwner: string, errorMessage: string): Promise<void> {
    const result = await this.prisma.eventOutbox.updateMany({
      where: {
        id,
        claimOwner,
