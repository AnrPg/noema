...
Noema is a **doctoral-level research platform** for metacognitive learning,
built as an **agent-first, API-first, microservices architecture**. It combines
spaced repetition, knowledge graphs, and LLM agents to create a formal epistemic
transformation system with adaptive state transitions.

**This is not a flashcard app. This is a cognitive operating system for
learning.**
...
- **Events Publishes**: `session.started`, `attempt.recorded`,
  `session.completed`
- **Agent Tools**: `startSession`, `recordAttempt`, `endSession`,
  `getSessionState`
...
- **Agent Tools**: `addNode`, `addEdge`, `queryGraph`, `proposeMutation`,
  `validateProposal`
...
- **Agent Tools**: `getLoadout`, `applyStrategy`, `evaluateStrategy`,
  `switchLoadout`
...
- **Agent Tools**: `processImage`, `transcodeAudio`, `extractText`,
  `generateThumbnail`
...
3. **Ontology Verification** - Class hierarchy, domain/range, disjointness,
   acyclicity
4. **UNITY Invariant Framework** - No circular prerequisites, no contradictions
5. **TLA+ Commit Modeling** - Safety (no invariant violations), Liveness
   (proposals commit)
...
**Critical Rule:** Higher layers depend on lower layers. Lower layers NEVER
...
**Scope Dimensions:** Attempt → Item-subset → Cluster → Domain → Session →
Profile

**Duration Types:** Time-based, Count-based, Stability-based, Manual,
Resolution-based
...
1. **FSRS v6.1.1** (recommended) - Adaptive, 19 parameters, stability/difficulty
   tracking
...
All documents are design specifications. This is a comprehensive architectural
blueprint for a doctoral-level research platform. Implementation will proceed in
...
1. **Structure over Content** - Focus on how knowledge is organized and
   understood
...
- `Noema_v2_0_knowledge_graph.docx` - Dual-graph architecture, guardrails,
  mathematical foundations
- `Noema_v2_0_mental_debugger.docx` - 7-frame traces, failure taxonomy,
  remediation cards
- `Noema_v2_0_strategy_loadouts.docx` - Cognitive control policies, force levels
- `Noema_v2_0_metacognition_overview.docx` - 10 training methods, 8 feature
  layers
...
**Last Updated:** 2024 **Architecture Status:** Design Complete, Implementation
Pending **Contact:** [Project Lead]
