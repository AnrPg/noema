...
This specification defines the REQUIRED structure for all agent tools using the
Model Context Protocol (MCP) format. All tools MUST conform to this
specification to ensure consistency across services and enable proper agent
orchestration.
...
Noema uses 10 LLM agents that interact with 15 microservices through
standardized tools. Each tool is a discrete capability that an agent can invoke.
Tools must return structured responses that guide agents on what to do next
(agentHints).

---

**Note:** This is the MINIMUM structure. Add tool-specific validation, caching
logic, retry mechanisms, circuit breakers, or any domain-specific functionality
as needed.
...
- **handler** (async function, REQUIRED)
  - Signature:
    `(params: TParams, context: ToolContext) => Promise<ToolResult<TResult>>`
  ...
  **REQUIRED CORE FIELDS:**
  - **suggestedNextActions** (array, REQUIRED)
    ...
    - Each action MUST have:
      ...
      - category (enum, optional): 'exploration' | 'optimization' | 'correction'
        | 'learning'
  ...
  - **relatedResources** (array, REQUIRED)
    ...
    - Each resource MUST have:
      ...
      - metadata (object, optional): category, lastAccessed, accessCount,
        quality, freshness
  ...
  - **riskFactors** (array, REQUIRED)
    ...
    - Each risk factor MUST have:
      - type (string): 'performance' | 'accuracy' | 'cost' | 'complexity' |
        'user-experience'
  ...
  - **preferenceAlignment** (array, REQUIRED)
    ...
    - Each alignment MUST have:
      ...
      - alignment (enum): 'strong' | 'moderate' | 'weak' | 'neutral' |
        'conflict'
  ...
  **OPTIONAL FIELDS (Strongly Recommended):**
  ...
  - **warnings** (array, OPTIONAL)
    - Each warning MUST have:
      - type (enum): 'validation' | 'duplicate' | 'conflict' | 'deprecation' |
        'performance'
  ...
  - **constraints** (array, OPTIONAL)
    ...
    - Example: ["Must complete within current session", "Cannot exceed 30
      minutes"]
