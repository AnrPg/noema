...
Neo4j excels at graph traversal, path queries, cycle detection, and neighborhood
operations — the bread and butter of a knowledge graph. PostgreSQL excels at
relational workflow data: the CKG mutation typestate pipeline (with its audit
log, state machine, and structured query needs), cached structural metrics, and
the relational misconception pattern/intervention tables.

This is the **Database per Service** pattern from DESIGN_PATTERNS — each service
picks the optimal store(s) for its domain.
...
- **Phase 5** (PKG Operations) and **Phase 6** (CKG Mutation Pipeline) are
  **independent peers** — neither depends on the other. They are sequenced 5 → 6
  by convention, not by necessity.
- **Phase 7** (Structural Metrics) **depends on both**: 7 of 11 metrics require
  PKG↔CKG comparison, so both graph types must be operational. Placing metrics
  after both eliminates the need for graceful degradation — every metric
  computes at full accuracy from day one.
