...
- Template and CKG mutation screens now show plain-language failure states instead of backend-oriented wording.
- Error handling now distinguishes between offline/network failures, timeouts, permissions, missing data, rate limits, invalid requests, conflicts, and server-side failures.
- Local-development hints remain available without exposing backend jargon in the main user-facing message.
