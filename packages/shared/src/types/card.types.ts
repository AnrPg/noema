...
export interface AudioCardContent {
  readonly type: 'audio';
  readonly mode: 'listen_recall' | 'pronunciation' | 'dictation' | 'sound_recognition';
...
export interface ErrorItem {
  readonly id: string;
  readonly location: {                          // Where the error is
...
export interface ConceptGraphContent {
  ...
  readonly mode: 'build_links' | 'label_links' | 'find_missing' | 'validate_graph';
 ...
 */
export interface MathBlock { //TODO: add support for various LaTex variations
  readonly id: string;
...
export interface CardSRSState {
  ...
  readonly difficulty: number;                  // Card difficulty (1-10)
