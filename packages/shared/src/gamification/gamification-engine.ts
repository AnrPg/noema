...
const LEVELS: Level[] = [
  ...
  { level: 2, name: 'Beginner', minXP: 100, maxXP: 250, perks: [], badge: '🌿' },
  { level: 3, name: 'Apprentice', minXP: 250, maxXP: 500, perks: [], badge: '🌲' },
  { level: 4, name: 'Learner', minXP: 500, maxXP: 850, perks: [], badge: '📚' },
  { level: 5, name: 'Scholar', minXP: 850, maxXP: 1300, perks: [], badge: '🎓' },
  { level: 6, name: 'Adept', minXP: 1300, maxXP: 1900, perks: [], badge: '⭐' },
  { level: 7, name: 'Expert', minXP: 1900, maxXP: 2700, perks: [], badge: '🌟' },
  { level: 8, name: 'Master', minXP: 2700, maxXP: 3800, perks: [], badge: '💫' },
  { level: 9, name: 'Grandmaster', minXP: 3800, maxXP: 5200, perks: [], badge: '🔥' },
  { level: 10, name: 'Legend', minXP: 5200, maxXP: 7000, perks: [], badge: '👑' },
  { level: 11, name: 'Mythic', minXP: 7000, maxXP: 9500, perks: [], badge: '🏆' },
  { level: 12, name: 'Transcendent', minXP: 9500, maxXP: 12500, perks: [], badge: '💎' },
  { level: 13, name: 'Enlightened', minXP: 12500, maxXP: 16500, perks: [], badge: '✨' },
  { level: 14, name: 'Sage', minXP: 16500, maxXP: 21500, perks: [], badge: '🧙' },
  { level: 15, name: 'Polymath', minXP: 21500, maxXP: Infinity, perks: [], badge: '🌌' },
];
...
export const DEFAULT_SKILL_TREES: SkillTree[] = [
  {
    ...
    edges: [
      { fromNodeId: 'spaced_repetition_basics', toNodeId: 'active_recall', isRequired: true },
      { fromNodeId: 'active_recall', toNodeId: 'interleaving', isRequired: true },
      { fromNodeId: 'active_recall', toNodeId: 'elaboration', isRequired: true },
      { fromNodeId: 'interleaving', toNodeId: 'metacognition', isRequired: false },
      { fromNodeId: 'elaboration', toNodeId: 'metacognition', isRequired: false },
    ],
  },
];
...
export class XPEngine {
  ...
  calculateLevel(totalXP: number): LevelInfo {
    ...
    const xpForNextLevel = currentLevel.maxXP === Infinity
  ...
  calculateReviewXP(params: {
    ...
    const { rating, cardDifficulty, responseTimeMs, streakDays = 0, isNewCard = false } = params;
    ...
    const difficultyBonus = 1 + (cardDifficulty - 0.5) * (this.config.difficultyMultiplier - 1);
    xp *= difficultyBonus;

    // Efficiency multiplier (faster response = more XP, up to a point)
    if (responseTimeMs && responseTimeMs < 10000) {
      const efficiencyBonus = 1 + (1 - responseTimeMs / 10000) * (this.config.efficiencyMultiplier - 1);
      xp *= efficiencyBonus;
    }
...
export class AchievementEngine {
  checkAchievements(stats: UserStats, unlockedAchievements: string[]): Achievement[] {
  ...
  private isAchievementMet(achievement: Achievement, stats: UserStats): boolean {
    const req = achievement.requirement;
    ...
    switch (req.type) {
      ...
      case 'accuracy':
        return (stats.retentionRate || 0) * 100 >= req.rate && (stats.totalReviews || 0) >= req.minReviews;
      case 'calibration':
...
export class StreakEngine {
  calculateStreak(lastActivityDate: Date | null, currentStreak: number): {
    newStreak: number;
    ...
    const lastActivityDay = new Date(lastDate.getFullYear(), lastDate.getMonth(), lastDate.getDate());
...
export class SkillTreeEngine {
  upgradeNode(
    ...
    const hasPrerequisites = prerequisites.every(prereq => unlockedNodes.includes(prereq));
    if (!hasPrerequisites) {
      return { success: false, xpSpent: 0, effects: [] };
    }
...
export class CalibrationEngine {
  calculateCalibrationScore(data: CalibrationData[]): CalibrationResult {
    ...
    return {
      ...
      overconfidenceBias: Math.round(overconfidence / calibrationBuckets.length),
      underconfidenceBias: Math.round(underconfidence / calibrationBuckets.length),
    };
  }
  ...
  calculateMemoryIntegrity(input: MemoryIntegrityInput): MemoryIntegrityResult {
    const factors = [
      ...
      { name: 'Review Consistency', score: input.reviewConsistency * 100, weight: 0.2 },
      { name: 'Mastery Progress', score: input.masteredRatio * 100, weight: 0.25 },
      { name: 'Memory Stability', score: Math.min(100, input.avgStability * 2), weight: 0.15 },
      { name: 'Recall Variance', score: Math.max(0, 100 - input.recallVariance * 100), weight: 0.1 },
    ];
...
export class LeaderboardEngine {
  ...
  getLeaderboardPosition(userId: string, entries: LeaderboardEntry[]): {
    rank: number;
    percentile: number;
  } {
    ...
    const percentile = Math.round((1 - (entry.rank - 1) / entries.length) * 100);
...
export class MetaLearningEngine {
  checkUnlocks(stats: UserStats, level: number, currentUnlocks: string[]): MetaLearningUnlock[] {
    ...
    for (const unlock of META_LEARNING_UNLOCKS) {
      ...
      switch (unlock.requirement.type) {
        case 'level':
          ...
          break;
        case 'streak':
          met = (stats.currentStreak || 0) >= (unlock.requirement.value as number);
          break;
        case 'mastered_cards':
          met = (stats.masteredCards || 0) >= (unlock.requirement.value as number);
          break;
      }
...
export class GamificationManager {
  ...
  processReview(input: ReviewInput, context: ReviewContext): GamificationResult {
    // Calculate XP
    ...
    let newCombo = isCorrect ? context.currentCombo + 1 : 0;
