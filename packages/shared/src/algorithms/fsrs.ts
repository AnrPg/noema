...
import { RatingValues, DEFAULT_FSRS_CONFIG, DEFAULT_FSRS_WEIGHTS } from '../types';
...
export class FSRSScheduler {
  ...
  private scheduleNewCard(now: Date): SchedulingResult {
    ...
    return {
      ...
      goodInterval: this.createPrediction('good', goodInterval, goodS, goodD, now),
      easyInterval: this.createPrediction('easy', easyInterval, easyS, easyD, now),
    };
  }
  ...
  private scheduleReviewCard(
    ...
    const hardD = this.nextDifficulty(lastD, RATINGS.hard);
    const hardS = this.nextRecallStability(lastD, lastS, retrievability, RATINGS.hard);
    
    const goodD = this.nextDifficulty(lastD, RATINGS.good);
    const goodS = this.nextRecallStability(lastD, lastS, retrievability, RATINGS.good);
    
    const easyD = this.nextDifficulty(lastD, RATINGS.easy);
    const easyS = this.nextRecallStability(lastD, lastS, retrievability, RATINGS.easy);
    ...
    return {
      ...
      againInterval: this.createPrediction('again', 0, againS, againD, now),
      hardInterval: this.createPrediction('hard', hardInterval, hardS, hardD, now),
      goodInterval: this.createPrediction('good', goodInterval, goodS, goodD, now),
      easyInterval: this.createPrediction('easy', easyInterval, easyS, easyD, now),
    };
  }
  ...
  private updateReviewCard(
    ...
    if (rating === 'again') {
      ...
      newStability = this.nextRecallStability(lastD, lastS, retrievability, ratingNum);
      newState = 'review';
      interval = this.nextInterval(newStability);
    }
  ...
  private nextDifficulty(d: number, rating: number): number {
    ...
    const revertedD = this.meanReversion(this.initDifficulty(RATINGS.easy), nextD);
  ...
  private nextRecallStability(
    ...
    const newS = s * (1 + Math.exp(this.w[8]) *
      (11 - d) *
      Math.pow(s, -this.w[9]) *
 ...
 */
export function createFSRSScheduler(config?: Partial<FSRSConfig>): FSRSScheduler {
 ...
 */
export function calculateMemoryIntegrityScore(cards: {
  stability: number;
  difficulty: number;
  ...
  const avgStability = reviewCards.reduce((sum, c) => sum + c.stability, 0) / reviewCards.length;
  const avgRetrievability = reviewCards.reduce((sum, c) => sum + c.retrievability, 0) / reviewCards.length;
