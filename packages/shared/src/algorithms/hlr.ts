...
export class HLRScheduler implements IScheduler {
  ...
  public updateAfterReview(
    ...
    const currentHalfLife =
      currentState.halfLife && currentState.halfLife > 0
        ? currentState.halfLife
        : this.config.baseHalfLife;

    // Update half-life based on rating
    const newHalfLife = this.updateHalfLife(currentHalfLife, isCorrect, rating);
    ...
    const newDifficulty = this.updateDifficulty(
    ...
    const algorithmData = {
      ...
      hlrVersion: "1.0.0",
      correctCount:
        ((currentState.algorithmData?.correctCount as number) || 0) +
        (isCorrect ? 1 : 0),
      incorrectCount:
        ((currentState.algorithmData?.incorrectCount as number) || 0) +
        (isCorrect ? 0 : 1),
   ...
   */
  public calculateRetrievability(
    elapsedDays: number,
    halfLife: number,
  ): number {
  ...
  private scheduleExistingCard(
    ...
    const currentHL =
      state.halfLife && state.halfLife > 0
        ? state.halfLife
        : this.config.baseHalfLife;
