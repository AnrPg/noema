...
export async function gamificationRoutes(app: FastifyInstance) {
  ...
  app.post('/streak/freeze', {
    ...
    return { message: 'Streak freeze used', remainingFreezes: streak.freezeCount - 1 };
  });
  ...
  app.post('/skills/:treeId/:nodeId/upgrade', {
    onRequest: [authenticate],
    schema: {
      tags: ['Gamification'],
  ...
  }, async (request: FastifyRequest<{ Params: { treeId: string; nodeId: string } }>, reply: FastifyReply) => {
  ...
  app.get('/challenges', {
    ...
    const daily = activeChallenges.filter((c) => 
      (c.challengeData as any).type === 'daily'
    );
    const weekly = activeChallenges.find((c) => 
      (c.challengeData as any).type === 'weekly'
    );
  ...
  app.get('/calibration', {
    ...
    if (reviews.length < 10) {
      return {
        score: null,
        message: 'Need at least 10 confidence-rated reviews to calculate calibration',
    ...
    const calibrationScore = calibrationEngine.calculateCalibrationScore(calibrationData);
  ...
  app.get('/memory-integrity', {
    ...
    const memoryIntegrity = calibrationEngine.calculateMemoryIntegrity({
      ...
      reviewConsistency,
      masteredRatio: stats.totalCards > 0 ? stats.masteredCards / stats.totalCards : 0,
  ...
  app.get('/meta-learning', {
    ...
    const unlockedMap = new Map(userUnlocks.map((u) => [u.unlockId, u.unlockedAt]));
