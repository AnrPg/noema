...
const startSessionSchema = z.object({
  ...
  sessionType: z.enum(['normal', 'cram', 'filtered', 'challenge']).default('normal'),
...
export async function studyRoutes(app: FastifyInstance) {
  ...
  app.get('/queue', {
    ...
    const { deckId, limit = 50 } = request.query as { deckId?: string; limit?: number };
  ...
  app.patch('/sessions/:id/end', {
    ...
    const avgResponseTime = cardsStudied > 0
      ? Math.round(session.reviews.reduce((sum, r) => sum + r.responseTime, 0) / cardsStudied)
      : 0;
  ...
  app.get('/today', {
    ...
    const [todayReviews, todayXP, remainingNew, remainingDue] = await Promise.all([
