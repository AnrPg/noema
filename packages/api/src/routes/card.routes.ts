...
const querySchema = z.object({
  ...
  orderBy: z.enum(['createdAt', 'nextReviewDate', 'difficulty', 'position']).default('position'),
...
export async function cardRoutes(app: FastifyInstance) {
  ...
  app.get('/', {
    ...
    if (query.dueBefore) where.nextReviewDate = { lte: new Date(query.dueBefore) };
  ...
  app.get('/:id', {
    ...
    const card = await prisma.card.findFirst({
      ...
      include: {
        ...
        relatedCards: {
          include: {
            targetCard: { select: { id: true, cardType: true, content: true } },
          },
        },
      },
    });
