  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {NODE_TYPES.map((type) => {
        ...
        const color: string = NODE_TYPE_COLOR[normalizedType] ?? '#6b7280';
        const isHidden = hiddenTypes.has(normalizedType);
        return (
          <button
            key={type}
            type="button"
            onClick={() => {
              onToggleType(normalizedType);
            }}
            className={[
          ...
          >
            ...
            />
            <span className="capitalize text-foreground">{normalizedType}</span>
          </button>
        );
      })}
    </div>
