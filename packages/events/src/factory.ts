...
export class EventBuilder<TPayload = unknown> {
  ...
  private _metadata: Partial<EventMetadata> = {};
   ...
   * Build the event.
   */
  build(): BaseEvent<TPayload> {
    if (!this._eventId) throw new Error('Event ID is required');
    if (!this._eventType) throw new Error('Event type is required');
    if (!this._aggregateType) throw new Error('Aggregate type is required');
    if (!this._aggregateId) throw new Error('Aggregate ID is required');
    if (this._payload === undefined) throw new Error('Payload is required');
    if (!this._metadata.serviceName) throw new Error('Service name is required');
    if (!this._metadata.serviceVersion)
      throw new Error('Service version is required');
    if (!this._metadata.environment) throw new Error('Environment is required');
    if (!this._metadata.correlationId)
